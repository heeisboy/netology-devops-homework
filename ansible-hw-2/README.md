...
Плейбук вызывает роль apache_status_role. Роль устанавливает веб-сервер apache, открывает 80 порт (если необходимо), запускает сервер и добавляет его в автозагрузку. Конфигурирует файл index.html с указанием характеристик управляемых узлов. Выводит информацию: CPU, RAM, величину первого HDD, IP-адрес. Перезапуск Apache только в случае изменения файла конфигурации apache. Производится проверка доступности веб-сайта с выводом ответа.
